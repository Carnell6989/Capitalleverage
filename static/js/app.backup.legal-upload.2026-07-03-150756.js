...
function ensureMyCaseAnswerActions() {
  const out = document.getElementById("mycase-answer");
  if (!out) return;

  if (document.getElementById("mycase-answer-actions")) return;

  const actions = document.createElement("div");
  actions.id = "mycase-answer-actions";
  actions.className = "mycase-answer-actions";
  actions.innerHTML = `
    <button onclick="copyTextById('mycase-answer')">Copy Answer</button>
    <button onclick="scrollMyCaseAnswerTop()">Top</button>
    <button onclick="scrollMyCaseAnswerBottom()">Bottom</button>
    <button onclick="saveMyCaseAnswer()">Save to My Case</button>
  `;

  out.insertAdjacentElement("afterend", actions);
}

function scrollMyCaseAnswerTop() {
  const out = document.getElementById("mycase-answer");
  if (out) out.scrollTop = 0;
}

function scrollMyCaseAnswerBottom() {
  const out = document.getElementById("mycase-answer");
  if (out) out.scrollTop = out.scrollHeight;
}

function saveMyCaseAnswer() {
  const out = document.getElementById("mycase-answer");
  if (!out) return;

  const saved = JSON.parse(localStorage.getItem("my_case_saved_answers") || "[]");
  saved.push({
    date: new Date().toISOString(),
    text: out.innerText
  });
  localStorage.setItem("my_case_saved_answers", JSON.stringify(saved));
  alert("Saved to My Case on this device.");
}

document.addEventListener("DOMContentLoaded", ensureMyCaseAnswerActions);

const oldAskMyCase = window.askMyCase;
window.askMyCase = async function() {
  if (typeof oldAskMyCase === "function") await oldAskMyCase();
  ensureMyCaseAnswerActions();
  scrollMyCaseAnswerTop();
};

const oldAskMyCaseAllAgents = window.askMyCaseAllAgents;
window.askMyCaseAllAgents = async function() {
  if (typeof oldAskMyCaseAllAgents === "function") await oldAskMyCaseAllAgents();
  ensureMyCaseAnswerActions();
  scrollMyCaseAnswerTop();
};

const oldResearchMyCase = window.researchMyCase;
window.researchMyCase = async function() {
  if (typeof oldResearchMyCase === "function") await oldResearchMyCase();
  ensureMyCaseAnswerActions();
  scrollMyCaseAnswerTop();
};

const oldRunMyCaseTask = window.runMyCaseTask;
window.runMyCaseTask = async function(taskName) {
  if (typeof oldRunMyCaseTask === "function") await oldRunMyCaseTask(taskName);
  ensureMyCaseAnswerActions();
  scrollMyCaseAnswerTop();
};

/* STRONG COPY FIX FOR MOBILE + SAFARI */
...
async function copyTextById(id) {
  ...
  try {
    ...
    try {
      ...
      alert("Copy failed. Press and hold the text, then copy manually.");
    }
...
function addProSeLawCenter() {
  if (document.getElementById("prose-law-center")) return;

  const firstPanel = document.querySelector(".page.active .panel") || document.querySelector(".panel");
  if (!firstPanel) return;

  const box = document.createElement("div");
  box.id = "prose-law-center";
  box.className = "prose-law-center";
  box.innerHTML = `
    <div class="prose-hero">
      <p class="eyebrow">Capital Leverage</p>
      <h1>Promote Pro Se Power</h1>
      <p>
        Fight smarter against creditors, collectors, landlords, corporations, schools,
        and systems. Capital Leverage helps you find laws, organize evidence, draft documents,
        and build leverage.
      </p>
    </div>

    <div class="law-grid">
      <button onclick="openLawFinder('Credit / FCRA')">💳 Credit / FCRA Laws</button>
      <button onclick="openLawFinder('Housing / Tenant Rights')">🏠 Housing Laws</button>
      <button onclick="openLawFinder('Consumer Protection')">⚖️ Consumer Protection</button>
      <button onclick="openLawFinder('Debt Collection / FDCPA')">📞 Debt Collection</button>
      <button onclick="openLawFinder('Federal Court / Pro Se')">🏛️ Federal Court Rules</button>
      <button onclick="openLawFinder('Business / Contracts')">💼 Business / Contracts</button>
    </div>
  `;

  firstPanel.before(box);
}

function openLawFinder(category) {
  let modal = document.getElementById("law-finder-modal");

  if (!modal) {
    modal = document.createElement("div");
    modal.id = "law-finder-modal";
    modal.className = "law-finder-modal";
    modal.innerHTML = `
      <div class="law-finder-box">
        <button class="modal-close" onclick="closeLawFinder()">×</button>
        <h2>Find Laws + Build Leverage</h2>
        <p>Select a state and explain your situation. Capital Leverage will search the web and break down the laws.</p>

        <input id="law-category" placeholder="Category">
        <input id="law-state" placeholder="State, example: Indiana">
        <textarea id="law-question" placeholder="Tell Capital Leverage what happened and what law or strategy you need..."></textarea>

        <button onclick="runLawSearch()">Find Laws</button>
        <pre id="law-answer"></pre>
      </div>
    `;
    document.body.appendChild(modal);
  }

  document.getElementById("law-category").value = category || "";
  document.getElementById("law-state").value = "";
  document.getElementById("law-question").value = "";
  document.getElementById("law-answer").innerText = "";
  modal.style.display = "flex";
}

function closeLawFinder() {
  const modal = document.getElementById("law-finder-modal");
  if (modal) modal.style.display = "none";
}

async function runLawSearch() {
  const category = document.getElementById("law-category").value;
  const state = document.getElementById("law-state").value;
  const question = document.getElementById("law-question").value;
  const out = document.getElementById("law-answer");

  if (!state.trim() || !question.trim()) {
    out.innerText = "Add a state and explain what happened first.";
    return;
  }

  out.innerText = "Searching laws and building leverage...";
  ...
  const res = await fetch("/ask", {
    ...
    body: JSON.stringify({
      mode: "research",
      message: `Find current laws, rules, agency guidance, and practical leverage for this situation.
...
Return:
1. Plain-English law summary
2. State-specific laws/rules to verify
3. Federal laws if relevant
4. What facts matter
5. Evidence needed
6. What letter, complaint, motion, or next step to use
7. Sources/URLs used`
    })
  });

  const data = await res.json();
  out.innerText = data.answer || "No answer returned.";
}

document.addEventListener("DOMContentLoaded", addProSeLawCenter);

function openBusinessLeveragePlan() {
  const msg = document.getElementById("msg");
  if (!msg) return;

  msg.value = `Build a 60-day Capital Leverage business funding readiness strategy.
...
Plan must include:
1. Credit dispute strategy
2. CFPB complaint strategy if needed
3. Certified mail letter plan
4. Business credit setup
5. Dun & Bradstreet setup checklist
6. Business plan outline
7. Tradeline/vendor account options to research
8. Calendar timeline for the next 60 days
9. Documents needed
10. Funding readiness steps

Do not guarantee approval. Give a lawful strategy to improve funding readiness.`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!business || document.getElementById("business-leverage-card")) return;

  const card = document.createElement("div");
  card.id = "business-leverage-card";
  card.className = "panel";
  card.innerHTML = `
    <h3>💼 Capital Leverage Business Funding Strategy</h3>
    <p>
      Even with a 500 credit score, Capital Leverage can build a 60-day business funding readiness plan.
      The strategy can help organize disputes, CFPB complaints, certified-mail letters, business credit setup,
      D&B steps, vendor accounts, business plans, and funding documents.
    </p>
    <button onclick="openBusinessLeveragePlan()">Build 60-Day Funding Strategy</button>
  `;

  business.prepend(card);
});

/* BUSINESS LEVERAGE BUILDER UPGRADE */

function openBusinessIndustry(industry) {
  const name = document.querySelector("#business input")?.value || "";
  const details = document.querySelector("#business textarea")?.value || "";
  const out = document.querySelector("#business pre");

  if (!out) return;

  out.innerText = "Building " + industry + " business funding strategy...";
  ...
  fetch("/ask", {
    ...
    body: JSON.stringify({
      mode: "research",
      message: `Build a Capital Leverage business funding plan.

Industry: ${industry}
Business name: ${name}
User details: ${details}

Build this like a real funding roadmap.

Return:
1. Best business model for this industry
2. Startup structure
3. Credit score target
4. What to do if credit score is 500
5. What to do if credit score is 650
6. What to do if credit score is 750
7. Business credit setup
8. Dun & Bradstreet checklist
9. Vendor/tradeline strategy to research
10. Banks/lenders to research
11. 60-day calendar plan
12. Documents needed
13. Business plan outline
14. Funding package checklist
15. Next 10 actions

Do not guarantee approval. Give a practical funding readiness strategy.`
    })
  })
  .then(res => res.json())
  .then(data => {
...
function addBusinessIndustryBar() {
  const business = document.getElementById("business");
  if (!business || document.getElementById("business-industry-bar")) return;

  const bar = document.createElement("div");
  bar.id = "business-industry-bar";
  bar.className = "business-industry-bar";
  bar.innerHTML = `
    <h3>💼 Business Funding Categories</h3>
    <p>Pick a business type and Capital Leverage will build the funding strategy automatically.</p>

    <div class="business-chip-grid">
      <button onclick="openBusinessIndustry('Construction')">🏗️ Construction</button>
      <button onclick="openBusinessIndustry('Restaurant / Food')">🍔 Restaurant</button>
      <button onclick="openBusinessIndustry('Trucking / Transportation')">🚚 Trucking</button>
      <button onclick="openBusinessIndustry('Cleaning Company')">🧹 Cleaning</button>
      <button onclick="openBusinessIndustry('Social Media / Creator Business')">📱 Social Media</button>
      <button onclick="openBusinessIndustry('Real Estate / Flipping')">🏠 Real Estate</button>
      <button onclick="openBusinessIndustry('Beauty / Barbershop / Salon')">💈 Beauty</button>
      <button onclick="openBusinessIndustry('Healthcare / Wellness')">🩺 Healthcare</button>
      <button onclick="openBusinessIndustry('Landscaping')">🌱 Landscaping</button>
      <button onclick="openBusinessIndustry('Consulting / Service Business')">📊 Consulting</button>
    </div>
  `;

  business.prepend(bar);
}

async function buildBusinessPlan() {
  const inputs = document.querySelectorAll("#business input");
  const textarea = document.querySelector("#business textarea");
  const out = document.querySelector("#business pre");

  const businessName = inputs[0]?.value || "";
  const businessType = inputs[1]?.value || "";
  const details = textarea?.value || "";

  if (!out) return;

  out.innerText = "Building your Capital Leverage business plan...";
  ...
  const res = await fetch("/ask", {
    ...
    body: JSON.stringify({
      mode: "research",
      message: `Build a complete Capital Leverage business funding plan.
...
2. Business model
3. Funding goal
4. Credit readiness plan
5. Business credit setup
6. D&B setup checklist
7. Vendor/tradeline strategy to research
8. Lenders/banks to research
9. 60-day action calendar
10. Documents needed
11. Business plan outline
12. Funding package checklist
13. Next steps

Do not guarantee approval. Build a funding readiness strategy.`
    })
  });
...
document.addEventListener("DOMContentLoaded", addBusinessIndustryBar);

function setCommandAgent(agent) {
  const sel = document.getElementById("command-agent");
  if (sel) sel.value = agent;
}

async function runCommandCenter() {
  const agent = document.getElementById("command-agent")?.value || "research";
  const details = document.getElementById("command-details")?.value || "";
  const out = document.getElementById("command-answer");

  if (!out) return;

  if (!details.trim()) {
    out.innerText = "Tell Capital Leverage what is going on first.";
    return;
  }

  out.innerText = "Capital Leverage is building your strategy...";
  ...
  const res = await fetch("/ask", {
    ...
    body: JSON.stringify({
      mode: agent,
      message: `You are Capital Leverage Command Center.

User selected agent: ${agent}

User situation:
${details}

Respond like a confident action plan. Do not say "Direct Answer."
Use this style:
- Here is the move
- We are going to
- Upload this
- Send this to this agent
- Next action

Return:
1. Capital Leverage strategy
2. Which agent handles each part
3. What documents/screenshots to upload
4. What letters or complaints to draft
5. What calendar deadlines/tasks to create
6. Next 7 actions
