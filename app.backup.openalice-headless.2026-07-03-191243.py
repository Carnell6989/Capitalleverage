...
def _openalice_login_session():
    ...
    try:
        r = sess.post(login_url, json=payload, timeout=30)
    except Exception as e:
        return None, f"OpenAlice login request failed: {e}"
...
def _openalice_chat(prompt_text):
    import os
    from dotenv import load_dotenv
                ...
                if isinstance(data, dict):
                    ...
                    for key in ["response", "message", "answer", "text", "content"]:
                        if key in data and data[key]:
                            return {"success": True, "response": data[key], "raw_json": data}
                    return {"success": True, "raw_json": data}
                return {"success": True, "raw": str(data)}
            else:
...
def trading_openalice_ping():
    return jsonify({
        "success": True,
        "openalice_base_url": openalice_base(),
        "token_present": bool(openalice_token())
    })
...
def trading_openalice_strategy():
    ...
    if not prompt:
        prompt = (
            "Here is the move. Find the strongest stocks for today, include GBPJPY research mode, "
            "keep risk at 1%, and give me exact levels to watch."
        )

    result = _openalice_chat(prompt)
...
def trading_openalice_watchlist():
    ...
    if not prompt:
        prompt = (
            "Build me a watchlist for the best names to paper trade today. "
            "Include SPY, QQQ, NVDA, TSLA, AAPL, AMD, META, MSFT, BTC/USD and GBPJPY research mode. "
            "Keep all risk planning at 1% max stop loss."
        )
...
def trading_openalice_journal():
    ...
    if not prompt:
        prompt = "Build me a trading journal template for paper trades."

    full_prompt = trading_lab_system_prompt(
        f"""
Build a trading journal / review response.
...
- take profit used
- lesson learned
- whether the trade followed the 1% risk rule
        """.strip(),
        mode="journal"
    )
    result = call_openalice(full_prompt)
    return jsonify(result)
...
def trading_openalice_setup():
    ...
    if not prompt:
        prompt = "Set up OpenAlice inside Capital Leverage Trading Lab for paper trading and GBPJPY research mode."

    full_prompt = trading_lab_system_prompt(
        f"""
OpenAlice setup request:

{prompt}

Explain:
- what OpenAlice will do inside Capital Leverage
- how Alpaca paper fits in
- how GBPJPY research mode fits in
- what to click next
- what the user should watch today
        """.strip(),
        mode="setup"
    )
    result = call_openalice(full_prompt)
...
@app.route("/trading/openalice/login-test", methods=["GET"])
def trading_openalice_login_test():
    sess, err = _openalice_login_session()
    if err:
        return jsonify({"success": False, "message": err})
    return jsonify({"success": True, "message": "OpenAlice login session created"})
