...
def trading_openalice_strategy():
    ...
    if not prompt:
        return jsonify({"success": False, "message": "Missing strategy prompt."}), 400

    full_prompt = trading_lab_system_prompt(prompt, mode="strategy")
...
def trading_openalice_watchlist():
    ...
    if not prompt:
        prompt = "Build me a paper-trading watchlist for the strongest U.S. stocks plus GBPJPY in research mode."

    full_prompt = trading_lab_system_prompt(
        f"""
Create a watchlist for me.

User goal:
{prompt}

Requirements:
- Give me 10 stock names maximum for Alpaca paper trading
- Include why each one made the list
- Include GBPJPY separately as a research-mode pair
- Tell me which 3 names are the strongest right now
- Keep risk at 1%
        """.strip(),
        mode="watchlist"
    )
